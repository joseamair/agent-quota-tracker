# 📸 Screenshots & Media Assets

Place your terminal screenshots and UI captures in this directory so they can be embedded directly into `README.md` and project documentation.

### Recommended File Names & Formats:
- **`terminal-status.png`**: Screenshot of `agents --status` showing the formatted table across all accounts.
- **`terminal-watchdog.png`**: Screenshot of `agents --poke-watch` showing the countdown timer and watchdog cycle.
- **`terminal-poke.png`**: Screenshot of `agents --poke` showing active verification and model replies.
- **`dashboard-preview.png`**: Screenshot of the local web dashboard (`http://localhost:5050`) showing glassmorphism cards and SVG rings.
- **`poke-cat.gif`**: Playful GIF of gently poking a cat to wake it up (featured in the Smart Poke section!).

Format: PNG or WebP recommended.
